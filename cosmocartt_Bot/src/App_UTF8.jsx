...
// import { CATEGORIES } from './data/mockData'; // DELETED
import { api } from './api';
    ...
    <div className="bg-white border border-purple-100 rounded-xl overflow-hidden mt-4 w-full max-w-md shadow-sm">
        ...
        <div className="divide-y divide-purple-50 text-sm">
            <div className="grid grid-cols-3 p-3 hover:bg-purple-50">
                ...
                <span className="text-center font-bold text-purple-900">Gé¦{products[0].price}</span>
                <span className="text-center font-bold text-purple-900">Gé¦{products[1].price}</span>
            </div>
            <div className="grid grid-cols-3 p-3 hover:bg-purple-50">
        ...
        <div className="bg-white border border-purple-200 rounded-xl overflow-hidden mt-4 w-full max-w-lg shadow-lg ring-1 ring-black/5">
                    ...
                    <tbody className="divide-y divide-purple-50">
                        {data.items.map((item, idx) => (
                            <tr key={idx} className="hover:bg-purple-50/50 transition-colors">
                                ...
                                <td className="py-3 px-4 text-right font-bold text-purple-700">Gé¦{(item.price * item.quantity).toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div className="bg-purple-50 p-4 border-t border-purple-200">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-2xl font-extrabold text-purple-900">Gé¦{data.total.toLocaleString()}</span>
                </div>
...
const OrderPreviewTable = ({ initialItems, onConfirm }) => {
    ...
    return (
        <div className="bg-white border border-purple-200 rounded-xl overflow-hidden mt-4 w-full max-w-xl shadow-lg ring-1 ring-black/5">
                ...
                {items.map((item) => (
                    <div key={item.id} className={`flex items-center gap-3 p-3 border-b border-purple-50 transition-colors ${item.quantity === 0 ? 'opacity-50 bg-slate-50' : 'hover:bg-purple-50/50'}`}>
                        ...
                        <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                            ...
                            <div className="flex items-center gap-2"><span className="text-sm font-bold text-purple-900">Gé¦{item.price}</span>{item.discount > 0 && <span className="text-xs text-slate-400 line-through">Gé¦{item.originalPrice}</span>}</div>
                        </div>
            ...
            <div className="bg-purple-50 p-4 border-t border-purple-200">
                <div className="flex justify-between items-end mb-4"><div><p className="text-xs text-purple-500 font-semibold">Estimated Cost</p><p className="text-2xl font-extrabold text-purple-900">Gé¦{totalCost.toLocaleString()}</p></div><div className="text-right"><p className="text-xs text-purple-500 font-semibold">{totalCount} items selected</p></div></div>
                            ...
                            {cart.map(item => (
                                <div key={`${item.id}-${item.selectedWeight}`} className="flex gap-4 p-3 border border-purple-100 rounded-xl hover:border-purple-300 transition-colors bg-white shadow-sm">
                                    ...
                                    <div className="flex-1 flex flex-col justify-between py-1">
                                        <div><p className="font-bold text-purple-900">{item.name} <span className="text-xs text-purple-400">({item.selectedWeight || 'Std'})</span></p><p className="text-sm font-semibold text-purple-500">Unit: Gé¦{item.price}</p></div>
                                        <div className="flex items-center gap-3">
                                    ...
                                    <div className="font-extrabold text-purple-700 self-end text-lg">Gé¦{(item.price * item.quantity).toLocaleString()}</div>
                                </div>
                            ))}
                        </div>
                        {cart.length > 0 && <div className="border-t border-purple-200 p-6 bg-purple-50"><div className="flex justify-between items-end mb-6"><span className="text-purple-500 font-bold uppercase text-sm tracking-wide">Total Amount</span><span className="text-3xl font-extrabold text-purple-900">Gé¦{cartTotal.toLocaleString()}</span></div><button onClick={() => { setIsCartOpen(false); onSend("Proceed to Checkout"); }} className="w-full py-4 bg-purple-900 text-white rounded-xl font-bold text-lg shadow-lg hover:bg-purple-800 transition-all flex justify-between px-6 items-center active:scale-[0.99]"><span>Secure Checkout</span><ArrowRight size={22} /></button></div>}
                    </div>
                </div>
            )}
        </>
