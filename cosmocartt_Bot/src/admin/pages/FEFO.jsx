...
const FEFO = ({ products, onApplyDiscount }) => {
    ...
    // Real logic: Items expiring in 30 days or less, or low stock
    const criticalItems = products.filter(p => {
        if (discountedIds.has(p.id)) return False;

        // Simple expiry check (p.expiry_date is DD-MM-YYYY)
        if (!p.expiry_date) return p.stock > 0 && p.stock <= 10;

        try {
            const [d, m, y] = p.expiry_date.split('-').map(Number);
            const expiryDate = new Date(y, m - 1, d);
            const today = new Date();
            const diffDays = Math.ceil((expiryDate - today) / (1000 * 60 * 60 * 24));

            return diffDays <= 30 || (p.stock > 0 && p.stock <= 5);
        } catch (e) {
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {criticalItems.map(item => (
                                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 font-mono text-xs text-gray-400">#BT-{item.id}</td>
                                    <td className="px-6 py-4"><span className={`text-sm font-bold ${getDaysLeft(item.expiry_date).includes('Expired') || getDaysLeft(item.expiry_date).includes('In 1 Days') ? 'text-red-600' : 'text-amber-600'}`}>{getDaysLeft(item.expiry_date)}</span></td>
